...
void* player_thread(void* arg) {
	...
	while (!player->game->game_done) {
		...
		int player_is_game_master = pthread_barrier_wait(&player->game->barrier);
		if (player_is_game_master == PTHREAD_BARRIER_SERIAL_THREAD) {
			int max = 2;
			int alive = 0;
			int last = -1;
			int repeat_round = 1;
		...
		pthread_barrier_wait(&player->game->barrier);
	}
...
int main(int argc, char const* argv[]) {
	...
	pthread_barrier_init(&state.barrier, NULL, n);
